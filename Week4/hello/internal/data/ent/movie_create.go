...
func (mc *MovieCreate) SetGeners(s string) *MovieCreate {
	mc.mutation.SetGeners(s)
	return mc
}
...
func (mc *MovieCreate) check() error {
	...
	if _, ok := mc.mutation.Geners(); !ok {
		return &ValidationError{Name: "geners", err: errors.New(`ent: missing required field "Movie.geners"`)}
	}
	return nil
}
...
func (mc *MovieCreate) createSpec() (*Movie, *sqlgraph.CreateSpec) {
	...
	if value, ok := mc.mutation.Geners(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeString,
			Value:  value,
