...
func (m *MovieMutation) SetGeners(s string) {
	m.geners = &s
}

// Geners returns the value of the "geners" field in the mutation.
func (m *MovieMutation) Geners() (r string, exists bool) {
	v := m.geners
	if v == nil {
		return
	}
...
// OldGeners returns the old "geners" field's value of the Movie entity.
// If the Movie object wasn't provided to the builder, the object is fetched from the database.
// An error is returned if the mutation operation is not UpdateOne, or the database query fails.
func (m *MovieMutation) OldGeners(ctx context.Context) (v string, err error) {
	if !m.op.Is(OpUpdateOne) {
		return v, errors.New("OldGeners is only allowed on UpdateOne operations")
	}
	if m.id == nil || m.oldValue == nil {
		return v, errors.New("OldGeners requires an ID field in the mutation")
	}
	oldValue, err := m.oldValue(ctx)
	if err != nil {
		return v, fmt.Errorf("querying old value for OldGeners: %w", err)
	}
	return oldValue.Geners, nil
}

// ResetGeners resets all changes to the "geners" field.
func (m *MovieMutation) ResetGeners() {
	m.geners = nil
}
...
func (m *MovieMutation) Fields() []string {
	...
	if m.geners != nil {
		fields = append(fields, movie.FieldGeners)
	}
	return fields
}
